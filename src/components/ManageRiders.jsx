import React, { useState, useContext, useEffect } from 'react';
import { ThemeContext } from '../main';
import { Link } from 'react-router-dom';
import { getAuth } from 'firebase/auth';

const ManageRiders = () => {
  const { darkMode } = useContext(ThemeContext);
  const [bcNumber, setBcNumber] = useState('');
  const [status, setStatus] = useState({ message: '', isError: false });
  const [isLoading, setIsLoading] = useState(false);
  const [riders, setRiders] = useState([]);
  const [isLoadingRiders, setIsLoadingRiders] = useState(false);

  // Fetch riders on component mount
  useEffect(() => {
    fetchRiders();
  }, []);

  const fetchRiders = async () => {
    setIsLoadingRiders(true);
    try {
      const response = await fetch('/api/racers');
      const data = await response.json();
      
      if (!response.ok) {
        throw new Error(data?.message || response.statusText || 'Failed to fetch riders');
      }
      
      setRiders(data || []);
    } catch (error) {
      console.error('Error fetching riders:', error);
      setStatus({ message: error.message || 'Failed to fetch riders', isError: true });
    } finally {
      setIsLoadingRiders(false);
    }
  };
  
  const handleRemoveRider = async (bc) => {
    if (!window.confirm(`Are you sure you want to remove rider with BC number ${bc}?`)) {
      return;
    }
    
    try {
      const auth = getAuth();
      const user = auth.currentUser;
      
      if (!user) {
        throw new Error('You must be logged in to remove a rider');
      }
      
      const token = await user.getIdToken();
      const response = await fetch(`/api/racers/${bc}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
      
      const data = await response.json().catch(() => null);
      
      if (!response.ok) {
        throw new Error(data?.message || response.statusText || 'Failed to remove rider');
      }
      
      setStatus({ message: `Rider with BC number ${bc} removed successfully!`, isError: false });
      
      // Refresh the riders list
      fetchRiders();
    } catch (error) {
      console.error('Error removing rider:', error);
      setStatus({ message: error.message || 'Failed to remove rider', isError: true });
    }
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    
    if (!bcNumber.trim()) {
      setStatus({ message: 'Please enter a BC number', isError: true });
      return;
    }
    
    setIsLoading(true);
    setStatus({ message: '', isError: false });
    
    try {
      const auth = getAuth();
      const user = auth.currentUser;
      
      if (!user) {
        throw new Error('You must be logged in to add a rider');
      }
      
      const token = await user.getIdToken();
      const response = await fetch('/api/racers/add', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ bc: bcNumber.trim() }),
      });
      
      const data = await response.json().catch(() => null);
      
      if (!response.ok) {
        throw new Error(data?.message || response.statusText || 'Failed to add rider');
      }
      
      setStatus({ message: `Rider with BC number ${bcNumber} added successfully!`, isError: false });
      setBcNumber('');
      
      // Refresh the riders list
      fetchRiders();
    } catch (error) {
      setStatus({ message: error.message || 'Failed to add rider', isError: true });
    } finally {
      setIsLoading(false);
    }
  };

  return (
    <div className={`p-4 space-y-4 ${darkMode ? 'dark bg-gray-900 text-white' : 'bg-white'}`}>
      <div className={`border-l-4 border-blue-600 rounded shadow-md p-4 mb-6 mx-2 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
        <h3 className={`text-lg font-medium ${darkMode ? 'text-white' : 'text-gray-800'}`}>Manage Riders</h3>
        <p className={`text-sm leading-relaxed ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
          Add new riders by entering their British Cycling ID number or view existing riders.
          <Link to="/about" className="ml-1 text-blue-500 hover:underline">Learn more</Link> about how this works.
        </p>
      </div>
      
      {/* Add Rider Form */}
      <div className={`p-6 ${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-md mb-6`}>
        <h4 className={`text-md font-medium mb-4 ${darkMode ? 'text-white' : 'text-gray-800'}`}>Add New Rider</h4>
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <label htmlFor="bcNumber" className="block mb-2 text-sm font-medium">
              BC Number
            </label>
            <input
              type="text"
              id="bcNumber"
              value={bcNumber}
              onChange={(e) => setBcNumber(e.target.value)}
              className={`w-full p-2 border rounded-md ${darkMode ? 'bg-gray-700 border-gray-600' : 'bg-white border-gray-300'}`}
              placeholder="Enter BC number"
              disabled={isLoading}
            />
          </div>
          
          <button
            type="submit"
            disabled={isLoading}
            className={`px-4 py-2 rounded-md ${
              darkMode 
                ? 'bg-blue-600 hover:bg-blue-700 text-white' 
                : 'bg-blue-500 hover:bg-blue-600 text-white'
            } ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            {isLoading ? 'Adding...' : 'Add Rider'}
          </button>
        </form>
        
        {status.message && (
          <div className={`mt-4 p-3 rounded-md ${
            status.isError 
              ? (darkMode ? 'bg-red-900 text-red-200' : 'bg-red-100 text-red-800') 
              : (darkMode ? 'bg-green-900 text-green-200' : 'bg-green-100 text-green-800')
          }`}>
            {status.message}
          </div>
        )}
      </div>
      
      {/* Riders List */}
      <div className={`p-6 ${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-md`}>
        <h4 className={`text-md font-medium mb-4 ${darkMode ? 'text-white' : 'text-gray-800'}`}>Riders List</h4>
        
        {isLoadingRiders ? (
          <div className="text-center py-4">Loading riders...</div>
        ) : riders.length > 0 ? (
          <div className="overflow-x-auto">
            <table className={`min-w-full ${darkMode ? 'text-gray-200' : 'text-gray-700'}`}>
              <thead>
                <tr className={`${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
                  <th className="px-4 py-2 text-left">BC Number</th>
                  <th className="px-4 py-2 text-right">Actions</th>
                </tr>
              </thead>
              <tbody>
                {riders.map((rider) => (
                  <tr key={rider.bc} className={`border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                    <td className="px-4 py-2">{rider.bc}</td>
                    <td className="px-4 py-2 text-right">
                      <button
                        onClick={() => handleRemoveRider(rider.bc)}
                        className={`px-2 py-1 rounded-md text-sm ${
                          darkMode 
                            ? 'bg-red-600 hover:bg-red-700 text-white' 
                            : 'bg-red-500 hover:bg-red-600 text-white'
                        }`}
                      >
                        Remove
                      </button>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        ) : (
          <div className={`text-center py-4 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            No riders found. Add a rider using the form above.
          </div>
        )}
        
        <button
          onClick={fetchRiders}
          disabled={isLoadingRiders}
          className={`mt-4 px-4 py-2 rounded-md ${
            darkMode 
              ? 'bg-gray-600 hover:bg-gray-700 text-white' 
              : 'bg-gray-200 hover:bg-gray-300 text-gray-800'
          } ${isLoadingRiders ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
          {isLoadingRiders ? 'Refreshing...' : 'Refresh List'}
        </button>
      </div>
    </div>
  );
};

export default ManageRiders;